...
data['x'] = np.linspace(-5e3,5e3,nx).astype('float32')# - X_Velocity/2
data['y'] = np.linspace(-5e3,5e3,ny).astype('float32')# - Y_Velocity/2
...
for k in range(0,nz):
    zero_mask = np.array(((X_eff-g_mean)**2 + ((z[k]-gz_mean)/(X.max() - x.min()))**2)<0.4**2,dtype=int)
    data['rho'][:,:,k] -= zero_mask*g_amp*np.exp(-(X_eff-g_mean)**2/(2*g_std**2)-((z[k]-gz_mean)/(X.max() - x.min()))**2/(2*g_std**2))
...
nrrd_filename = '/home/xbao/LAB_test/shadow_piv_sim/photon/sample-data/piv/const_grad_piv_grad_x_%02d.nrrd' % grad_x
